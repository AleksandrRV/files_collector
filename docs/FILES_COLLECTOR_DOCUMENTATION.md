...
- путь выбранного `.gitignore` файла.
...
| `.gitignore file` | Путь к `.gitignore`, правила которого скрывают файлы и папки. `Select .gitignore...` выбирает файл, `Reset` сбрасывает опцию. |
...
Опция `.gitignore file` на вкладке `Filters` подключает внешний `.gitignore`. Всё, что
попадает под его правила, **скрывается полностью**:

- элементы не отображаются в дереве файлов и папок;
- файлы не попадают в план, Markdown, manifest и статистику (счётчики режимов,
  оценка размера, список расширений на вкладке `Formats`);
- проигнорированные папки не обходятся, поэтому их содержимое не читается.

В отличие от `Exclude patterns`, такие файлы не получают режим `Excluded` и не
учитываются как исключённые: для приложения их просто не существует.

| Элемент | Действие |
|---|---|
| `Select .gitignore...` | Выбирает файл через системный диалог. Повторный выбор того же файла перечитывает правила с диска. |
| `Reset` | Сбрасывает опцию; дерево, план и статистика возвращаются к полному составу. |

Под кнопками отображается состояние: число применённых правил, отсутствие файла или
ошибка чтения. Inventory cache остаётся полным снимком диска, поэтому счётчик в
`Inventory status` показывает все найденные файлы; фильтр применяется поверх него при
построении дерева, плана и статистики. Недоступный или удалённый `.gitignore` не прерывает работу — фильтр
просто не применяется, а причина выводится в статусе и в лог.

Поддерживается формат gitignore(5):

| Синтаксис | Значение |
|---|---|
| `# комментарий`, пустая строка | Игнорируется. |
| `bin` | Совпадает с файлом или папкой `bin` на любой глубине. |
| `/build` | Привязка к каталогу, где лежит `.gitignore`. |
| `cache/` | Только каталоги. |
| `*.log`, `file?.txt`, `temp[0-9].tmp` | Wildcard-и в пределах одного сегмента пути. |
| `**/generated/**` | Произвольное число промежуточных каталогов. |
| `!keep.log` | Отрицание: возвращает файл обратно. |
| `\#notes.txt` | Экранирование спецсимвола. |

Как и в Git, решает **последнее** совпавшее правило, а отрицание не может вернуть
файл из уже проигнорированного каталога. Пути в правилах отсчитываются от каталога,
в котором лежит выбранный `.gitignore`; если файл находится вне Scan Root, правила
применяются от корня сканирования.

Выбранный путь сохраняется в пресете (поле `scan.gitIgnorePath`), поэтому при
переключении и загрузке пресета фильтр восстанавливается автоматически.
...
```json
{
  ...
  "scan": {
    ...
    "gitIgnorePath": "D:\\Project\\.gitignore"
  },
  "extensions": [
    {
      "extension": ".png",
...
5. Поле `gitIgnorePath` необязательно: оно отсутствует в файле, когда опция не
   используется. Путь абсолютный, поэтому при переносе пресета на другую машину его
   нужно указать заново или сбросить кнопкой `Reset`.
